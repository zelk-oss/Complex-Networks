...
int main()
{
    ...
    int m_parameter = 3;
    double prob_destruction = (N_quad*m+m*m+1)/(N_quad*(N_quad-1));
    ...
    std::string filename_entropy = "BA_entropy_n_pattern.csv"; // create the output file
    //file_entropy.open(filename_entropy, std::fstream::out);
    std::cout << "parto\n";

    for (int m_parameter = 3; m_parameter < 50; m_parameter++)
    {
        ...
        for (int a = 1; a <= N_generations; a++) // set number of generations
        {
            ...
            hopfield.Ba_network(m_parameter); // I dilute the network

            // I count all the occurencies for a singles weight
            for (int i = 0; i < N_link; i++)
            {
                int bin = static_cast<int>(hopfield.Get_Weight(i) * N_quad * (1 - prob_destruction)) + N_pattern;
        ...
        file_entropy << m_parameter << ';' << N_pattern << ',' << entropy_Np << '\n';
        file_entropy.close();
